...
<div id="content">
  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
 	...
 	<div class="form-group">
 	   <label class="col-sm-2 control-label" for="input-parent"><span data-toggle="tooltip" title="<?php echo $help_article_parent; ?>"><?php echo $entry_article_parent; ?></span></label>
	     <div class="col-sm-10">
	 	<input type="text" name="article_path" value="<?php echo $article_path; ?>" id="input-article-parent" class="form-control" />
	 	<input type="hidden" name="article_parent_id" value="<?php echo $article_parent_id; ?>" />
	    </div>
 	</div>
...
<script type="text/javascript"><!--
$('input[name=\'article_path\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=extension/module/latest_articles/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				json.unshift({
					article_id: 0,
	...
	'select': function(item) {
		$('input[name=\'article_path\']').val(item['label']);
		$('input[name=\'article_parent_id\']').val(item['value']);
	}
});
//--></script>
