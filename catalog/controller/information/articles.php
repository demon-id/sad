...
class ControllerInformationArticles extends Controller {
	public function index() {
		...
		if ($this->config->get('article_module_description' .(int)$this->config->get('config_language_id'))) {
			$articles_description = $this->config->get('article_module_description' .(int)$this->config->get('config_language_id'));
		...
		foreach ($articles as $result) {
			...
			if (strlen(strip_tags($short_description)) > 20) {
				...
				if (strlen($description) > 20) {
					$desc = utf8_substr(str_replace(array('<p>', '</p>'), '', $description), 0, 300) . '...';
				}else{
					$desc = '';
				}
			}
